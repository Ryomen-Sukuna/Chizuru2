...
def sblacklist(update, context):
    ...
    if conn:
        chat_id = conn
        chat_name = bot.getChat(conn).title
    else:
...
@user_admin
@typing_action
def sblacklist_mode(update, context):
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    msg = update.effective_message  # type: Optional[Message]
    bot, args = context.bot, context.args
...
@kigmsg(((Filters.sticker) & (Filters.chat_type.groups)), group=BLACKLIST_GROUP)
@user_not_admin
def del_sblacklist(update, context):
    bot = context.bot
    chat = update.effective_chat  # type: Optional[Chat]
